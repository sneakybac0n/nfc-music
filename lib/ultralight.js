...
class MifareUltralight {
	...
	async fastRead(startPage, endPage) {
		...
		const response = await this.reader.transmit(cmd, length || 7);
...
nfc.on('reader', async reader => {
	...
	reader.on('card', async card => {
      ...
      console.log(' nfcCard.hasNDEFMessage() ', nfcCard.hasNDEFMessage());
        ...
        nfcCard.hasReadPermissionshasReadPermissions()
      );

			const data = await ultralight.fastRead(
        4,
        nfcCard.getNDEFMessageLengthToRead()
      );

			let payload = nfcCard.parseNDEF(data);
