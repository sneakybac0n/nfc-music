...
nfc.on('reader', async reader => {
	...
	reader.on('card', async card => {
		...
		try {
			...
			let payload;
			try {
				const NDEFlibMessage = new ndeflib.NdefMessage.fromByteArray(payload);
				console.log('NDEFlibMessage', NDEFlibMessage);

				payload = nfcCard.parseNDEF(data, { ignoreEmptyTLV: true, ignoreUnknownTLV: true, ignoreUnparseableTLV: true, ignoreInvalidTLVLength: true });
			} catch (err) {
