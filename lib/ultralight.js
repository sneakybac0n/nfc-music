...
const { NFC, TAG_ISO_14443_3, TAG_ISO_14443_4, KEY_TYPE_A, KEY_TYPE_B, TransmitError } = require('nfc-pcsc');
...
class MifareUltralight {
	...
	async fastRead(startPage, endPage) {
		...
		const response = await this.reader.transmit(cmd, length || 10);
...
nfc.on('reader', async reader => {
	...
	reader.on('card', async card => {
		...
		// console.info('card detected', reader, card);
		...
		try {
			...
			console.info('passwordAuthenticate: successfully authenticated');

		} catch (err) {
			console.error('passwordAuthenticate error:', err);
		}

		try {
			const data = await ultralight.fastRead(4, 40);
			console.log('data', data);
			let payload;

			try {
				const t = data.toString('ascii').replaceAll("~=", "").trim();
				console.log("test", t)
				const [_, message] = t.split('en');
				console.log("message", message)
				// remove all spaces and only keep letters and numbers and semi for spotify
				const cleanMessage = message.replace(/[^\x00-\x7F]/g, "").replaceAll("=", "").replaceAll("~", "").replaceAll(" ", "").replaceAll(";", ":");

				if (cleanMessage.includes("spotify:")) {
					process_sonos_command(cleanMessage);
					return false
				}
			} catch (e) {
			...
			try {
				payload = nfcCard.parseNDEF(data);
			} catch (err) {
				console.error('parseNDEF error:', err);
			}
			...
			if (payload) {
				for (const record of payload) {
					switch (record.type) {
						...
						case 'text':
							const received_text = record.text?.trim();
			...
			// const NDEFRawMessage = await ultralight.reader.read(
			// 	4, 
			// 	nfcCard.getNDEFMessageLengthToRead()
			// );

			// console.log('NDEFRawMessage:', NDEFRawMessage);
			// console.log('NDEFRawMessage2:', NDEFRawMessage.toString('utf8'));
			
		} catch (err) {
			console.error('fastRead error:', err);
			return;
		}
		...
		//
		// } catch (err) {
		// 	console.error('configuration write error:', err);
		// }
