...
module.exports = async function process_sonos_command(received_text) {
  ...
  let received_text_lower = received_text.toLowerCase().split('|');
  console.info(received_text_lower);
  
  let command_text = received_text_lower[0];
  let shuffle = received_text_lower[1].includes('shuffle');
  let keep_queue = received_text_lower[1].includes('keep_queue');
