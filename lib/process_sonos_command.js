...
export default async function process_sonos_command(received_text) {
  ...
  } else if (received_text_lower.startsWith('say:')) {
    service_type = 'say';
    sonos_instruction = 'say/' + received_text.slice(4);
  ...
  if (!service_type) {
    console.log(
      'Service type not recognised. Text should begin ' +
        "'spotify', 'tunein', 'amazonmusic', 'apple'/'applemusic', 'command', 'http', 'playlist', 'say', or 'room'."
    );
    return;
  }
